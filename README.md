...
5. 生成された PDF をサーバーの `generated/` フォルダに保存
6. ユーザーに PDF 表示・ダウンロードのリンクを提供

**データ保存について**:
- このアプリケーションはデータベースを使用していません
- 入力されたデータは PDF 形式でのみ保存され、元のフォームデータは永続化されません
- 生成された PDF はまずサーバー側の `generated/` フォルダに保存されます（サーバーのファイルシステム上）
- 「表示」ボタンをクリックするとブラウザで PDF が開きます（サーバーからの読み取り）
- 「ダウンロード」ボタンをクリックするとユーザーのコンピュータのデフォルトダウンロードフォルダに PDF がコピーされます
...
|                                                       |
|  +-------------------------+   +--------------------+ |
|  | invoice_20241206.pdf   |   | invoice_20241205.pdf| |
|  | 作成日時: 2024/12/06   |   | 作成日時: 2024/12/05| |
|  |                        |   |                     | |
|  | [表示] [ダウンロード]  |   | [表示] [ダウンロード]| |
|  +-------------------------+   +--------------------+ |
|                                                       |
|  +-------------------------+   +--------------------+ |
|  | invoice_20241204.pdf   |   | invoice_20241203.pdf| |
|  | 作成日時: 2024/12/04   |   | 作成日時: 2024/12/03| |
|  |                        |   |                     | |
|  | [表示] [ダウンロード]  |   | [表示] [ダウンロード]| |
|  +-------------------------+   +--------------------+ |
|                                                       |
...
│   └── css/
│       └── style.css      # カスタム CSS
├── generated/             # 生成された PDF の保存フォルダ（サーバー側）
├── requirements.txt       # 依存パッケージリスト
└── README.md              # プロジェクト説明（最終更新: 2024/12/06）
```
